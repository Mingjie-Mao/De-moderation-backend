...
src/test/java/                        3.5k 行 · 27 个文件 · 130 个测试
...
130 个测试。集成测试通过 Testcontainers 启动自己的 PostgreSQL，所以不需要 Compose 那套在跑。用真数据库而不是内存替代品，是因为这个 schema 的正确性活在部分索引、check 约束和唯一索引里，内存引擎并不强制这些——包括让并发举报塌缩成单个案件的那一条，由一个同时发两条举报的测试覆盖。
